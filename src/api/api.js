...
export default {
  ...
  getNovelListByCat (gender, type, major, minor = '', start = 0, limit = 20) {
    return Vue.http.get('/api/book/by-categories?gender=' + gender + '&type=' + type + '&major=' + major + '&minor=' + minor + '&start=' + start + '&limit=' + limit)
  },
  ...
  /**
     * 获取小说源(正版源)
     * @param {String} bookId 小说id
     * 'http://api.zhuishushenqi.com/toc?view=summary&book=548d9c17eb0337ee6df738f5'
     */
  getGenuineSource (bookId) {
    return Vue.http.get(`/api/toc?view=summary&book=${bookId}`)
  },
  ...
  /**
     ...
     */
  getMixSource (bookId) {
    return Vue.http.get(`/api/atoc/?view=summary&book=${bookId}`)
  },
  ...
  getMixChapters (bookId) {
    return Vue.http.get('/api/mix-atoc/' + bookId + '?view=chapters')
  },
  ...
  getChapters (sourceId) {
    return Vue.http.get('/api/atoc/' + sourceId + '?view=chapters')
  },
  ...
  getBookChapterContent (chapterUrl) {
    return Vue.http.get(`/content/chapter/${chapterUrl}`)
  },
  ...
  fuzzySearch (searchWord) {
    return Vue.http.get('/api/book/fuzzy-search?query=' + searchWord)
  },
